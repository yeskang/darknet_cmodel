...
__global__ void scale_bias_kernel(float *output, float *biases, int n, int size)
{
	...
	// syoh, quant candidate 2
    if(offset < size){
		/*
		float aval = output[(batch*n+filter)*size + offset]; // 5-3
		float bval = biases[filter]; // 3-5
		if (aval >= 15.875)
			aval = 15.875;
		else if (aval <= -16)
			aval = -16;
		if (bval >= 3.96875)
			...
			bval = -4;
		int8_t qa = ((int8_t)(aval*8 + 0.5));	// -2^7  ~
		int8_t qb = ((int8_t)(bval*32 + 0.5));		// -2^7  ~
		int16_t qmult = qa*qb;					// -2^14 ~ --> -2^6
		float mult = (float)qmult / (1<<8);
		
		output[(batch*n+filter)*size + offset] = mult;
		*/
...
__global__ void add_bias_kernel(float *output, float *biases, int batch, int n, int size)
{
	...
	// syoh, quantized addition of bias
	float q_bias = biases[j];
	float sum = output[(k*n+j)*size + i];
	if (q_bias >= 7.937500)  // 4-4
		q_bias = 7.937500;
	else if (q_bias <= -8)
		q_bias = -8;
	int8_t qb = (int8_t)(q_bias*16);

	// integer ACC
	int32_t qacc = (int32_t)(sum*(1<<16));
	qacc = (int32_t)qb*(1<<12) + qacc;
	sum  = (float) qacc / (1<<16);
    output[(k*n+j)*size + i] = sum;
}
...
__global__ void normalize_kernel(int N, float *x, float *mean, float *variance, int batch, int filters, int spatial)
{
    ...
    // syoh, quant candidate 1
	/*
    float scaled_val = (x[index] - mean[f])/(sqrtf(variance[f] + .00001f));
	int8_t qval  = ((int8_t)(scaled_val*16+0.5));	// 4-4
    x[index] = (float) qval / 16.0;
	*/
